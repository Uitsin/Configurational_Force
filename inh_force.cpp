...
const double Xdim = 0.102;   // just arbitrary
const double Ydim = 0.102;

const int X = 86;   // size of (0.5, 0.866) direction
const int Y = 86;  // size of (0.5, -0.866) direction
...
const int ER = 6;      // Right Edge
const int EL = -6;     // Left Edge
...
void Msum (double** uijCHX, double** uijCHY, double** Ouijx, double** Ouijy, int* LengthH, int NP, double a, double* Ax, double* Ay, double* TBW) {       // Arg: (uijx, uijy, Ouijx, Ouijy, &fn[0], NP, a, &Ax[0], &Ay[0], M)
     ...
     for (int k = 0; k < NP; k++) {
         ...
         for (int j = 0; j < *(LengthH + k); j++) {
             ...
             TBW[k] = TBW[k] +  pow( *(uijCHX[k] + j)*Ax[k] + *(uijCHY[k] + j)*Ay[k] , 2) - pow( *(Ouijx[k] + j)*Ax[k] + *(Ouijy[k] + j)*Ay[k] , 2);
...
double GoUValue_CL (double F, double K, double M, double FuComp, double KuComp, double MuComp, double m, double A, double B, double valley) {           //Just for calculation
    ...
    double UI1 = 0.3*m*A; 
    double UI2 = 0;
    double UI4 = 2*m*A*0.09*( 2*M+1 - 1/(2*M+1) )*( 0.1*(sqrt(2*M+1)-6) + 0.045*(sqrt(2*M+1)-6)*(sqrt(2*M+1)-6) );
...
*/
    ...
    double* M = (double*) malloc (NP* sizeof(double));
...
//    KuComponent(uijy, &fn[0], a, NP, uijx, yKu);
    ...
    Msum(uijx, uijy, Ouijx, Ouijy, &fn[0], NP, a, &Ax[0], &Ay[0], M);


    for (int k = 0; k < NP; k++) {

            double LS;    // Local Stretch; local area expansion ratio
            double area[5] = {0.083333, 0.25, 0.416666, 0.666666, 1};          // original K values for fn = 2, 3, 4, 5, 6 respectively.
            
            if ( fn[k] == 0 ) { LS = 0; }
            else if ( fn[k] == 1 ) { LS = pow( ( ux[*(*(nnl+k))]*Ax[k] + uy[*(*(nnl+k))]*Ay[k] ), 2 )/(a*a); }
            else { LS = K[k]/area[fn[k]-2]; }
            
            if ( ( abs( LS - 1*stchX*stchX*stchY*stchY ) > 0.001 ) && AFF[k] == 2 ) { AFF[k] = 1; }
            if ( LS < CRIT*CRIT ) { AFF[k] = 0; }
            if ( (LS > CRIT*CRIT)&& (AFF[k] != 2) ) { AFF[k] = 1; } 
            
//            if ( AFF[k] == 1) { valley[k] = sqrt(K[k]); } 
                 
                 
                GoUx[k] = ( (AFF != 0) ) 
                        ? GoUValue_CL(F[k], K[k], M[k], xFu[k], xKu[k], xMu[k], m, A, B, valley[k])
                        : GoUValue_RX(F[k], K[k], M[k], xFu[k], xKu[k], xMu[k], m, A, B, valley[k]);
      
                GoUy[k] = ( (AFF != 0) ) 
                        ? GoUValue_CL(F[k], K[k], M[k], yFu[k], yKu[k], yMu[k], m, A, B, valley[k])
                        : GoUValue_RX(F[k], K[k], M[k], yFu[k], yKu[k], yMu[k], m, A, B, valley[k]);
             
/*
            GoUx[k] = (K[k] > CRIT*CRIT*1 || fn[k] == 6) 
                    ? GoUValue_CL(F[k], K[k], M[k], xFu[k], xKu[k], xMu[k], m, A, B)
                    : GoUValue_RX(F[k], K[k], M[k], xFu[k], xKu[k], xMu[k], m, A, B);
      
            GoUy[k] = (K[k] > CRIT*CRIT || fn[k] == 6)
                    ? GoUValue_CL(F[k], K[k], M[k], yFu[k], yKu[k], yMu[k], m, A, B)
                    : GoUValue_RX(F[k], K[k], M[k], yFu[k], yKu[k], yMu[k], m, A, B);
*/
...
*/           
        ...
        if ( (n%50000==0) || (n==T) || (n==T-1) || (n==5) || (n==100) || (n==1000) || (n==3000) ) {

                 ofstream fout ( (  string("updA_Avector")
                                  + string("_T")
                                  + int_to_string(n)
                                  + string(".txt")       ).c_str() 
                                 );
                 ...
                 for ( int k = 0; k < NP; k++ ) {
                     ...
                     fout << ux[k] << ' ' << uy[k] << ' ' << Ax[k] << ' ' << Ay[k] << ' ' << K[k] << ' ' << M[k] << endl;
    ...
    ofstream nnl_output( ( string("Model1_") + string("nnl_T") + int_to_string(T) + string(".txt") ).c_str() );
    ofstream fn_output( ( string("Model1_") + string("fn_T") + int_to_string(T) + string(".txt") ).c_str() );
